...
const Chip = memo(({
  ...
  const colors = {
    gray:    "dark:bg-gray-800/50 bg-gray-200 dark:text-gray-400 text-gray-600 dark:border-gray-700/50 border-gray-300",
    blue:    "bg-blue-500/20 dark:text-blue-300 text-blue-700 border-blue-500/30",
    amber:   "bg-amber-500/20 dark:text-amber-300 text-amber-700 border-amber-500/30",
    emerald: "bg-emerald-500/20 dark:text-emerald-300 text-emerald-700 border-emerald-500/30",
    purple:  "bg-purple-500/20 dark:text-purple-300 text-purple-700 border-purple-500/30",
    red:     "bg-red-500/20 dark:text-red-300 text-red-700 border-red-500/30",
  };
  return (
    <span className={cn(
...
const DataRow = memo(({
  ...
  return (
    <div className="flex items-center justify-between gap-4 py-2.5 border-b dark:border-white/5 border-gray-200 last:border-0">
      ...
      <span className={cn(
        ...
        accent ? accentColors[accent] : "dark:text-gray-200 text-gray-800"
      )}>
        {value}
      </span>
    </div>
...
const Card = memo(({
  title,
  icon: Icon,
  iconColor = "dark:text-gray-400 text-gray-600",
  children,
  className,
}: {
  title: string;
  ...
  <div className={cn("rounded-2xl border dark:border-white/10 border-gray-300/50 bg-white/[0.03] overflow-hidden", className)}>
    <div className="flex items-center gap-2 px-4 py-3 border-b dark:border-white/10 border-gray-300/50 bg-white/[0.03]">
      <Icon className={cn("w-4 h-4 shrink-0", iconColor)} />
      <span className="text-xs font-bold dark:text-gray-400 text-gray-600 uppercase tracking-widest">{title}</span>
    </div>
...
const ActionBtn = memo(({
  ...
  const variants = {
    ...
    secondary: "dark:bg-white/5 bg-gray-100 hover:dark:bg-white/10 hover:bg-gray-200 active:dark:bg-white/15 active:bg-gray-300 dark:text-gray-200 text-gray-800 border dark:border-white/10 border-gray-300/50",
    ghost:     "bg-transparent hover:dark:bg-white/5 hover:bg-gray-100 active:dark:bg-white/10 active:bg-gray-200 dark:text-gray-400 text-gray-600 border dark:border-white/5 border-gray-200",
  };
  return (
    <button
      type="button"
...
export const PDFPreviewView = memo(({
  ...
  const footerActions = (
    <footer className="px-5 pb-8 pt-3 dark:bg-gray-950/80 bg-gray-50 border-t dark:border-white/5 border-gray-200 flex-shrink-0">
      <div className={cn(
        "grid gap-2",
  ...
  return (
    <div className="flex flex-col h-full transform-gpu">
      {/* Header */}
      {!hideHeader && (
        <header className="px-5 py-3 border-b dark:border-white/5 border-gray-200 flex items-center gap-3 flex-shrink-0 dark:bg-gray-950/80 bg-gray-50">
        <button
          type="button"
          onClick={onBack}
          className="w-9 h-9 rounded-xl dark:bg-white/5 bg-gray-100 hover:dark:bg-white/10 hover:bg-gray-200 active:dark:bg-white/15 active:bg-gray-300 flex items-center justify-center transition-all touch-manipulation shadow-sm"
          aria-label="Volver"
        >
          <ChevronLeft className="w-5 h-5 dark:text-gray-300 text-gray-700" />
        </button>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold dark:text-white text-gray-900 uppercase tracking-wider leading-none">Vista Previa</p>
          <p className="text-xs font-medium text-gray-500 mt-1">Orden #{orden.idPersonalizado || orden.id?.slice(-6)}</p>
        </div>
        {state.status === "loading" && <Loader2 className="w-5 h-5 dark:text-blue-400 text-blue-700 animate-spin" />}
        {(state.status === "ready-web" || state.status === "ready-html" || state.status === "ready-native") && (
          <span className="text-xs dark:text-emerald-400 text-emerald-700 font-bold uppercase tracking-tight flex items-center gap-1.5 bg-emerald-500/10 px-2 py-1 rounded-md">
      ...
      {/* Área central */}
      <div className="flex-1 overflow-hidden dark:bg-gray-950 bg-gray-50 relative">
        {state.status === "loading" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-5">
            <div className="relative">
              <div className="w-16 h-16 rounded-2xl bg-blue-500/10 border border-blue-500/20 flex items-center justify-center">
                <FileText className="w-8 h-8 dark:text-blue-400 text-blue-700/80" />
              </div>
              <div className="absolute -bottom-1 -right-1 w-6 h-6 rounded-full dark:bg-gray-950 bg-gray-50 flex items-center justify-center shadow-lg">
                <Loader2 className="w-4 h-4 dark:text-blue-400 text-blue-700 animate-spin" />
              </div>
            </div>
        ...
        {state.status === "error" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-6 px-8 text-center">
            <div className="w-16 h-16 rounded-2xl bg-red-500/10 border border-red-500/20 flex items-center justify-center">
              <AlertCircle className="w-9 h-9 dark:text-red-400 text-red-700" />
            </div>
            <div>
            ...
            <div className="flex flex-col w-full max-w-xs gap-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                {onDownload && <ActionBtn variant="ghost" icon={Download} onClick={handleDownload} className="text-[10px] px-2 dark:text-gray-300 text-gray-700">Descargar</ActionBtn>}
              </div>
            </div>
          </div>
        )}
        ...
        {state.status === "ready-native" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-6 px-6 overflow-y-auto py-8">
            <div className="w-16 h-16 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center shadow-inner">
              <CheckCircle className="w-9 h-9 dark:text-emerald-400 text-emerald-700" />
            </div>
            <div className="text-center">
              <p className="text-xl font-bold dark:text-white text-gray-900 tracking-tight">PDF Generado</p>
              <p className="text-sm dark:text-gray-400 text-gray-600 mt-2 max-w-[260px] mx-auto leading-relaxed">
            ...
            <div className="w-full max-w-xs space-y-2">
              ...
              <div className="divide-y divide-white/5 rounded-2xl border dark:border-white/10 border-gray-300/50 bg-white/[0.03] overflow-hidden shadow-sm">
                {[
                  ...
                  <div key={label} className="flex justify-between items-center px-4 py-3">
                    ...
                    <span className="text-xs font-bold dark:text-gray-200 text-gray-800">{value}</span>
                  </div>
                ))}
              </div>
            </div>
...
const DetailView = memo(({
  ...
  return (
    <div className="flex flex-col h-full min-h-0 transform-gpu">
      ...
      <header className="px-5 pt-5 pb-4 flex items-start justify-between gap-4 flex-shrink-0 dark:bg-gray-950/50 bg-gray-50">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-3 mb-1.5 flex-wrap">
            <h2 className="text-xl font-black dark:text-white text-gray-900 leading-none tracking-tight uppercase">
        ...
        <button
          type="button"
          onClick={onClose}
          className="w-10 h-10 rounded-xl dark:bg-white/5 bg-gray-100 hover:dark:bg-white/10 hover:bg-gray-200 active:dark:bg-white/15 active:bg-gray-300 flex items-center justify-center transition-all touch-manipulation shrink-0 shadow-sm"
          aria-label="Cerrar"
        >
      ...
      <div className="h-px dark:bg-white/5 bg-gray-100 mx-5 mb-4" />
      ...
      >
        {/* CLIENTE */}
        <Card title="Cliente" icon={User} iconColor="dark:text-blue-400 text-blue-700">
          <div className="flex items-start justify-between gap-4 mb-4">
            <p className="text-base font-bold dark:text-white text-gray-900 leading-tight">{orden.cliente.name}</p>
            {orden.cliente.phone && (
              <a
                href={`tel:${orden.cliente.phone}`}
                className="flex items-center gap-1.5 text-xs dark:text-blue-400 text-blue-700 font-bold uppercase tracking-wider bg-blue-500/10 px-3 py-1.5 rounded-full shrink-0 border border-blue-500/20 active:scale-95 transition-transform"
              >
        ...
        {/* DISPOSITIVO */}
        <Card title="Dispositivo" icon={Cpu} iconColor="dark:text-emerald-400 text-emerald-700">
          <p className="text-base font-bold dark:text-white text-gray-900 mb-4 capitalize">{deviceName || "Equipo no especificado"}</p>
          <div className="space-y-0">
          ...
          {orden.contador && (
            <div className="mt-4 flex items-center justify-between px-4 py-3 rounded-xl bg-purple-500/5 border border-purple-500/10 shadow-inner">
              ...
              <span className="text-sm font-black dark:text-purple-300 text-purple-700">
          ...
          {orden.contadorMaquina !== undefined && (
            <div className="mt-2 flex items-center justify-between px-4 py-3 rounded-xl bg-blue-500/5 border border-blue-500/10 shadow-inner">
              ...
              <span className="text-sm font-black dark:text-blue-300 text-blue-700">
          ...
          <Card title="Garantía Aplicada" icon={ShieldCheck} iconColor="dark:text-amber-400 text-amber-700">
            <div className="space-y-4">
              <div>
                <p className="text-xs font-bold text-gray-600 uppercase tracking-widest mb-1.5">Orden de Referencia</p>
                <p className="text-lg font-black dark:text-amber-200 text-amber-800 font-mono tracking-tighter bg-amber-500/10 px-3 py-2 rounded-xl border border-amber-500/20 inline-block">
              ...
              <div>
                ...
                <div className="bg-white/[0.02] border dark:border-white/5 border-gray-200 rounded-2xl p-4 shadow-inner">
          ...
          {/* SERVICIO */}
        <Card title="Servicio realizado" icon={Wrench} iconColor="dark:text-orange-400 text-orange-700">
            ...
            <div>
              ...
              <div className="space-y-2">
                {orden.piezasUsadas.map((p, i) => (
                  <div key={i} className="flex items-center justify-between py-2 border-b dark:border-white/5 border-gray-200 last:border-0">
                    <span className="text-sm dark:text-gray-300 text-gray-700 font-medium">{p.pieza}</span>
                    <span className="text-xs font-black dark:text-blue-400 text-blue-700 bg-blue-500/10 px-2.5 py-1 rounded-lg border border-blue-500/20">×{p.cantidad}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {(!orden.tareasRealizadas || orden.tareasRealizadas.length === 0) &&
           (!orden.piezasUsadas || orden.piezasUsadas.length === 0) && (
            <div className="py-4 text-center rounded-xl bg-white/[0.02] border border-dashed dark:border-white/5 border-gray-200">
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={label}>
                  ...
                  <p className="text-sm dark:text-gray-300 text-gray-700 leading-relaxed font-medium">{value}</p>
                </div>
              ))}
              {orden.diagnosticoFinal && (
                <div className="bg-blue-500/5 border border-blue-500/15 rounded-2xl px-4 py-3.5 shadow-inner">
                  <p className="text-xs font-bold dark:text-blue-500 text-blue-600 uppercase tracking-widest mb-1.5">Diagnóstico final</p>
                  <p className="text-sm dark:text-blue-200 text-blue-800 leading-relaxed font-bold">{orden.diagnosticoFinal}</p>
                </div>
              )}
            </div>
          </Card>
        ...
        {(orden.garantiaHabilitada !== false && (orden.garantiaTiempoHasta || orden.garantiaDescripcion)) || (orden.garantiaHabilitada === false) ? (
          <Card title="Garantía de la orden" icon={ShieldCheck} iconColor="dark:text-amber-400 text-amber-700">
              ...
              <>
                <div className="flex items-center justify-between gap-4 py-1">
                  <div>
                    ...
                    <p className="text-base font-black dark:text-white text-gray-900">{formatFecha(orden.garantiaTiempoHasta)}</p>
                  </div>
                  {(() => {
                ...
                {orden.garantiaDescripcion && (
                  <div className="mt-4 p-3 rounded-xl bg-white/[0.02] border dark:border-white/5 border-gray-200">
                    <p className="text-xs dark:text-gray-400 text-gray-600 leading-relaxed font-medium">
                      {orden.garantiaDescripcion}
                    </p>
                  </div>
                )}
        ...
        {orden.firmaCliente && (
          <Card title="Conformidad del cliente" icon={PenLine} iconColor="text-indigo-400">
            <div className="flex items-center gap-6">
              <div className="bg-white rounded-2xl p-3 flex-shrink-0 shadow-xl border dark:border-white/10 border-gray-300/50">
                <img
              ...
              <div className="flex-1 min-w-0 space-y-2">
                <div>
                  ...
                  <p className="text-base font-black dark:text-white text-gray-900 leading-tight truncate">
                ...
                <div className="flex items-center gap-1.5 dark:text-emerald-400 text-emerald-700">
      ...
      <footer className="px-5 pt-3 pb-8 dark:bg-gray-950/80 bg-gray-50 border-t dark:border-white/5 border-gray-200 flex-shrink-0">
        <div className="grid grid-cols-2 gap-3">
          ...
          {onDownload && (
            <ActionBtn variant="ghost" icon={Download} onClick={handleDownload} className="col-span-full mt-1 border-dashed dark:text-gray-400 text-gray-600 hover:dark:text-gray-200 hover:text-gray-800">
              Descargar Documento
            </ActionBtn>
          )}
        </div>
  ...
  return createPortal(
    ...
    >
      <div
        ...
        className={cn(
          "dark:bg-gray-950 bg-gray-50 w-full flex flex-col overflow-hidden relative",
          "shadow-[0_0_50px_-12px_rgba(0,0,0,0.5)] border-t border-x dark:border-white/10 border-gray-300/50 sm:border",
